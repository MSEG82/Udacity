...
def write_to_csv(results, filename):
    ...
    corresponds to the information in a
    single close approach from the `results`
    ...
    with open(filename, 'w') as salida_csv:
        writer = csv.writer(salida_csv)
        writer.writerow(fieldnames)
        for row in results:
            if row.neo.name is not None:
                row.neo.name = row.neo.name
            else:
                row.neo.name = ''
...
def write_to_json(results, filename):
    ...
    their values and the 'neo' key mapping to a dictionary
    ...
    for row in results:
        if row.neo.name is not None:
            ...
            row.neo.name = ''
        if row.neo.hazardous:
            row.neo.hazardous = bool(1)
        else:
            row.neo.hazardous = bool(0)
        ...
        salida_json.append(r)

    with open(filename, 'w') as json_file:
        json.dump(salida_json, json_file, indent=4, sort_keys=False)
