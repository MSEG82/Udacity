"""A database encapsulating collections of near-Earth objects and their cls.
...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        resolves to an empty collection, and the `.neo`
        attribute of each `CloseApproach` is None.

        However, each `CloseApproach` has an attribute (`._designation`) that
        matches the `.designation` attribute of the corresponding NEO. This
        constructor modifies the supplied NEOs and close approaches
        to link them together - after it's done,
        the `.approaches` attribute of each NEO has a collection of
        that NEO's close approaches, and the `.neo` attribute of each
        ...
        self.name_dic = {}
        self.designation_dic = {}

        for i in range(len(neos)):
            self.name_dic[neos[i].name] = neos[i]
            self.designation_dic[neos[i].designation] = neos[i]

        for approach in self._approaches:
            neo = self.designation_dic[approach._designation]
            if neo is not None:
    ...
    def get_neo_by_designation(self, designation):
        ...
        or `None`.
        """
        return self.designation_dic.get(designation, None)
    ...
    def get_neo_by_name(self, name):
        ...
        """
        return self.name_dic.get(name, None)
    ...
    def query(self, filters=()):
        ...
        objects that match all of the provided filters.
        If no arguments are provided, generate all known
        close approaches.
        ...
        for approach in self._approaches:
            ...
            for filter in filters:
                if filter(approach) is not True:
                    control = False
                    break
            if control is True:
                yield approach
